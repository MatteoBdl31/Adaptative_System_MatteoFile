            ...
            try:
                ...
                for collaborative_trail in collaborative_trails:
                    ...
                    if trail_id in exact_suggestions_ids:
                        ...
                        # Still include it in collaborative section - collaborative recommendations should be shown
                        # even if they don't match all current filters, as they represent what similar users liked
                        collaborative_in_scored.append(collaborative_trail)
                ...
                # Add top 5 collaborative trails that are NOT already in exact/suggestions to suggestions
                # Track which trails we add to suggestions so we don't duplicate them in the collaborative section
                top_5_collaborative = collaborative_in_scored[:5]
                added_to_suggestions_ids = set()
                
                for collaborative_trail in top_5_collaborative:
                    trail_id = collaborative_trail.get("trail_id")
                    if trail_id in exact_suggestions_ids:
                        # Already in exact/suggestions - skip (will be marked as collaborative above)
                        continue
                    
                    if trail_id in scored_trails_map:
                        trail_to_add = scored_trails_map[trail_id].copy()
                    else:
                        # Trail not in scored_trails - use collaborative_trail data directly
                        trail_to_add = collaborative_trail.copy()
                        # Set a default relevance_percentage for ranking
                        trail_to_add["relevance_percentage"] = 50.0  # Default relevance for collaborative-only trails
                    ...
                    added_to_suggestions_ids.add(trail_id)
                ...
                # Add collaborative trails (that are NOT already in exact/suggestions AND NOT added to suggestions above) to the collaborative section
                # This ensures the separate "collaborative" section is populated without duplicates
                collaborative_trails_filtered = []
                for collaborative_trail in collaborative_in_scored:
                    trail_id = collaborative_trail.get("trail_id")
                    # Exclude trails that are already in exact/suggestions OR were just added to suggestions
                    if trail_id not in exact_suggestions_ids and trail_id not in added_to_suggestions_ids:
                        # Not in exact/suggestions and not added to suggestions - add to collaborative section
                        collaborative_trails_filtered.append(collaborative_trail)
                ...
                for collaborative_trail in collaborative_trails_filtered:
                    ...
                    # For trails not in scored_trails, we need to get the trail data
                    trail_id = collaborative_trail.get("trail_id")
                    if trail_id not in scored_trails_map:
                        from backend.db import get_trail
                        trail_data = get_trail(trail_id)
                        if trail_data:
                            # Merge trail data with collaborative metadata
                            collaborative_trail.update(trail_data)
                            collaborative_trail["is_collaborative"] = True
                            collaborative_trail["collaborative_avg_rating"] = collaborative_trail.get("collaborative_avg_rating")
                            collaborative_trail["collaborative_user_count"] = collaborative_trail.get("collaborative_user_count")
                ...
                # Continue without collaborative recommendations - this is not critical
                collaborative_trails = []
